...
static void Boot_App_SetSlotMetadata(uint8_t slot, uint32_t version, uint32_t crc32) {
    if (slot == SLOT_A) {
        g_boot_app.boot_info.version_a = version;
        g_boot_app.boot_info.app_a_crc = crc32;
    } else if (slot == SLOT_B) {
        g_boot_app.boot_info.version_b = version;
...
static void Boot_App_ArmPendingUpgrade(uint8_t target_slot, const BootManifestOperation *operation) {
    Boot_App_SetSlotMetadata(target_slot, operation->version, operation->crc32);
...
void Boot_App_Process(void) {
    ...
    switch (g_boot_app.state) {
        ...
        case BOOT_APP_STATE_MANIFEST_LOAD:
            ...
            g_boot_app.selected_operation =
                Boot_SimpleManifest_FindFirstSupported(&g_boot_app.manifest);
            ...
            LOG_INFO(BOOT_LOG_TAG, "Selected upgrade item: %s size=%lu crc=0x%08lX version=0x%08lX",
                     ...
                     (unsigned long) g_boot_app.selected_operation->version);
        ...
        case BOOT_APP_STATE_UPGRADE: {
            ...
            error = Boot_SimpleUpgrade_Run(g_boot_app.selected_operation, target_slot);
            if (error != BOOT_ERR_NONE) {
            ...
            Boot_App_ArmPendingUpgrade(target_slot, g_boot_app.selected_operation);
