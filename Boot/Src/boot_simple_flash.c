...
static const BootSimpleFlashRegion g_boot_app_region =
{
  BOOT_APP_BASE,
  BOOT_APP_SIZE
};
...
BootError Boot_SimpleFlash_EraseApp(void)
{
  ...
  if ((Boot_SimpleFlash_AddressToSector(g_boot_app_region.base, &start_bank, &start_sector) == false) ||
  ...
  error = Boot_Platform_FlashEraseSectors(start_bank,
                                          start_sector,
                                          (end_sector - start_sector) + 1U);
  if (error == BOOT_ERR_NONE)
  {
    Boot_Platform_FlashRefreshCache();
  }
