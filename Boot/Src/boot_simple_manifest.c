...
static BootError Boot_SimpleManifest_ParseString(const char **cursor,
                                                 const char *end,
                                                 char *buffer,
...
static BootError Boot_SimpleManifest_ParseU32Value(const char **cursor, const char *end, uint32_t *value)
{
  const char *position;
  char token[32];
  size_t length = 0U;
...
static BootError Boot_SimpleManifest_ParseOperation(const char *begin,
                                                    const char *end,
                                                    BootManifestOperation *operation)
{
  ...
  while (cursor < end)
  {
    ...
    {
      error = Boot_SimpleManifest_ParseString(&cursor,
                                              end,
                                              operation->file,
...
BootError Boot_SimpleManifest_Load(BootManifest *manifest)
{
  ...
  while (cursor < end)
  {
    ...
    object_end = cursor - 1;
    error = Boot_SimpleManifest_ParseOperation(object_begin,
                                               object_end,
...
const BootManifestOperation *Boot_SimpleManifest_FindFirstSupported(const BootManifest *manifest)
{
  ...
  for (index = 0U; index < manifest->count; ++index)
  {
    ...
    if ((operation->present_in_crc_dir != 0U) &&
        (strcmp(operation->file, "app.bin") == 0))
    {
      return operation;
    }
  }
