...
static BootError Boot_SimpleUpgrade_ReadHeader(BootPlatformFile *file, BootUpgradePacketHeader *header)
{
...
BootError Boot_SimpleUpgrade_Run(const BootManifestOperation *operation)
{
  ...
  for (attempt = 0U; attempt < BOOT_UPGRADE_MAX_RETRIES; ++attempt)
  {
    LOG_INFO(BOOT_LOG_TAG,
             "Upgrade %s attempt %lu/%u",
             operation->file,
             (unsigned long)(attempt + 1U),
             (unsigned int)BOOT_UPGRADE_MAX_RETRIES);

    last_error = Boot_SimpleUpgrade_RunSingleAttempt(operation);
    if (last_error == BOOT_ERR_NONE)
    {
      LOG_INFO(BOOT_LOG_TAG,
               "Upgrade %s success, size=%lu crc=0x%08lX",
               operation->file,
               (unsigned long)operation->size,
               (unsigned long)operation->crc32);
      return BOOT_ERR_NONE;
    }
