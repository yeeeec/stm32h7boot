...
#define BOOT_UDISK_FINGERPRINT_RAW_LENGTH (4U + 4U + BOOT_PLATFORM_USB_SERIAL_LENGTH + \
                                           BOOT_UDISK_VOLUME_ID_LENGTH + sizeof(BOOT_UDISK_CHECK_SALT))
...
static BootError Boot_Udisk_CalcFingerprint(const BootUdiskMediaInfo *info, uint32_t *fingerprint)
{
  ...
  length = snprintf(raw_text,
                    sizeof(raw_text),
                    "%s%s%s%s%s",
                    info->usb_vid,
                    info->usb_pid,
                    Boot_Udisk_NormalizeSerialView(info->usb_serial),
                    volume_id,
