...
firmware_status_t Composition_Init(void) {
    ...
    status = SpiNorBlockAdapter_Interface(&external_flash_adapter)
                 ->get_info(SpiNorBlockAdapter_Interface(&external_flash_adapter)->context,
                            &external_flash_info);
    if (!FirmwareStatus_IsOk(status)) {
        LOG_ERROR("composition", "external flash info failed: %d", (int) status);
        return status;
    }
    if ((external_flash_info.capacity_bytes == 0U) || (external_flash_info.write_size == 0U) ||
    ...
    if (!FirmwareStatus_IsOk(status)) {
        LOG_ERROR("composition", "runtime service init failed: %d", (int) status);
        return status;
    }

    status = Application_Configure(&runtime_service);
    if (!FirmwareStatus_IsOk(status)) {
        LOG_ERROR("composition", "application configure failed: %d", (int) status);
        return status;
    }
