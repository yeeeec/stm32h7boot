...
static firmware_status_t FinishFailure(at24_t *device, firmware_status_t status)
{
    firmware_status_t protect_status = SetWriteEnabled(device, 0);

    device->operation_state  = AT24_OPERATION_FAILED;
    device->operation_status = FirmwareStatus_IsOk(protect_status) ? status : protect_status;
