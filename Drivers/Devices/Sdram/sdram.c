    ...
    const sdram_config_t *config)
{
    ...
    if ((device == NULL) || (port == NULL) || (config == NULL) ||
        (port->send_command == NULL) || (port->set_refresh_rate == NULL) ||
        (port->delay_ms == NULL) || (config->startup_delay_ms == 0U) ||
        (config->auto_refresh_count == 0U) || (config->refresh_rate == 0U))
    {
     ...
     */
    status = device->port.send_command(
        device->port.context, SDRAM_COMMAND_CLOCK_ENABLE, 1U, 0U);
    ...
    status = device->port.send_command(
        device->port.context, SDRAM_COMMAND_PRECHARGE_ALL, 1U, 0U);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
    ...
    status = device->port.set_refresh_rate(
        device->port.context, config->refresh_rate);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
