...
static firmware_status_t ValidateRange(const spi_nor_t *device, uint32_t address, uint32_t size)
{
...
{
    return (device != NULL) && (device->operation_state == SPI_NOR_OPERATION_BUSY);
}


static firmware_status_t ReadStatus(spi_nor_t *device, uint8_t *status_register)
{
    spi_nor_transaction_t transaction = Transaction(SPI_NOR_COMMAND_READ_STATUS, 0U, 0U, 0U);
...
{
    spi_nor_transaction_t transaction = Transaction(SPI_NOR_COMMAND_WRITE_ENABLE, 0U, 0U, 0U);
    ...
    return ((status_register & SPI_NOR_STATUS_WRITE_ENABLE_LATCH) != 0U) ? FIRMWARE_STATUS_OK
                              ...
                              const spi_nor_config_t *config)
{
    ...
    device->erase_timeout_ms   = (config->erase_timeout_ms == 0U) ? SPI_NOR_DEFAULT_ERASE_TIMEOUT_MS
    ...
    if (((device->info.jedec_id[0] == 0U) && (device->info.jedec_id[1] == 0U) &&
         (device->info.jedec_id[2] == 0U)) ||
        ((device->info.jedec_id[0] == 0xFFU) && (device->info.jedec_id[1] == 0xFFU) &&
    ...
    device->address_bytes = (device->info.capacity_bytes > SPI_NOR_3BYTE_CAPACITY_LIMIT) ? 4U : 3U;
    ...
    {
        transaction = Transaction(SPI_NOR_COMMAND_ENTER_4BYTE_ADDRESS, 0U, 0U, 0U);
        status      = device->port.command(device->port.context, &transaction);
        if (!FirmwareStatus_IsOk(status))
        {
            return status;
...
firmware_status_t SpiNor_Read(spi_nor_t *device, uint32_t address, void *data, uint32_t size)
{
    ...
    while (remaining > 0U)
    {
        ...
        spi_nor_transaction_t transaction =
            Transaction(SPI_NOR_COMMAND_FAST_READ, address, device->address_bytes, 8U);
        firmware_status_t status =
            device->port.receive(device->port.context, &transaction, destination, chunk_size);
                                 ...
                                 uint32_t size)
{
    ...
    while (remaining > 0U)
    {
        uint32_t page_remaining  = device->info.page_size - (address % device->info.page_size);
        uint32_t chunk_size      = (remaining < page_remaining) ? remaining : page_remaining;
        firmware_status_t status = SpiNor_ProgramStart(device, address, source, chunk_size);
                                      ...
                                      uint32_t size)
{
    ...
    transaction = Transaction(SPI_NOR_COMMAND_PAGE_PROGRAM, address, device->address_bytes, 0U);
    status =
...
firmware_status_t SpiNor_Erase(spi_nor_t *device, uint32_t address, uint32_t size)
{
    ...
    if (((address % device->info.erase_size) != 0U) || ((size % device->info.erase_size) != 0U))
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
    }
    ...
    while (remaining > 0U)
    {
        firmware_status_t status = SpiNor_EraseStart(device, address, device->info.erase_size);
...
firmware_status_t SpiNor_EraseStart(spi_nor_t *device, uint32_t address, uint32_t size)
{
    ...
    transaction = Transaction(SPI_NOR_COMMAND_SECTOR_ERASE, address, device->address_bytes, 0U);
    status      = device->port.command(device->port.context, &transaction);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
