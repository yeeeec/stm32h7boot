...
static firmware_status_t Transmit(stm32_rom_boot_t *device, const uint8_t *data, uint32_t size,
                                  uint32_t timeout_ms)
{
    return device->port.transmit(device->port.context, data, size, timeout_ms);
}

static firmware_status_t Receive(stm32_rom_boot_t *device, uint8_t *data, uint32_t size,
                                 uint32_t timeout_ms)
{
    return device->port.receive(device->port.context, data, size, timeout_ms);
}

static firmware_status_t ReceiveAck(stm32_rom_boot_t *device, uint32_t timeout_ms)
{
    uint8_t response;
    ...
    return (response == STM32_ROM_BOOT_ACK_BYTE) ? FIRMWARE_STATUS_OK : FIRMWARE_STATUS_IO_ERROR;
}

static firmware_status_t SendCommand(stm32_rom_boot_t *device, uint8_t command)
{
    uint8_t frame[2] = {command, (uint8_t) ~command};
    firmware_status_t status =
        Transmit(device, frame, sizeof(frame), device->config.command_timeout_ms);
...
static firmware_status_t SendAddress(stm32_rom_boot_t *device, uint32_t address)
{
    uint8_t frame[5];

    PutBigEndian32(frame, address);
    frame[4] = XorBytes(frame, 4U);
    return Transmit(device, frame, sizeof(frame), device->config.command_timeout_ms);
}

static firmware_status_t ValidateInitialized(const stm32_rom_boot_t *device)
{
    if (device == NULL)
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
    }
    return (device->initialized != 0) ? FIRMWARE_STATUS_OK : FIRMWARE_STATUS_INVALID_STATE;
}

static firmware_status_t ValidateInBootloader(const stm32_rom_boot_t *device)
{
    ...
    return (device->in_bootloader != 0) ? FIRMWARE_STATUS_OK : FIRMWARE_STATUS_INVALID_STATE;
}

static int InfoContainsCommand(const stm32_rom_boot_info_t *info, uint8_t command)
{
    uint32_t index;
...
static firmware_status_t RestoreApplication(stm32_rom_boot_t *device)
{
    firmware_status_t status;
    ...
    {
        status =
            device->port.configure_uart(device->port.context, STM32_ROM_BOOT_UART_APPLICATION_MODE);
...
{
    if ((device == NULL) || (port == NULL) || (config == NULL) || (port->transmit == NULL) ||
        (port->receive == NULL) || (port->set_boot0 == NULL) || (port->reset_target == NULL) ||
        (port->delay_ms == NULL))
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
    }
    ...
    if ((config->command_timeout_ms == 0U) || (config->write_timeout_ms == 0U) ||
...
firmware_status_t Stm32RomBoot_Enter(stm32_rom_boot_t *device)
{
    ...
    if (device->port.configure_uart != NULL)
    {
        status = device->port.configure_uart(device->port.context, STM32_ROM_BOOT_UART_ROM_MODE);
        if (!FirmwareStatus_IsOk(status))
        {
            return status;
        }
    ...
    status = Transmit(device, &sync, 1U, device->config.command_timeout_ms);
    if (FirmwareStatus_IsOk(status))
    {
...
firmware_status_t Stm32RomBoot_GetInfo(stm32_rom_boot_t *device, stm32_rom_boot_info_t *info)
{
    ...
    status = Receive(device, &count_minus_one, 1U, device->config.command_timeout_ms);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
    ...
    status = Receive(device, &parsed.bootloader_version, 1U, device->config.command_timeout_ms);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
    ...
    {
        status = Receive(device, &parsed.commands[index], 1U, device->config.command_timeout_ms);
        if (!FirmwareStatus_IsOk(status))
        {
            return status;
        }
    ...
    }
    {
        ...
        status = Receive(device, id_bytes, sizeof(id_bytes), device->config.command_timeout_ms);
        if (!FirmwareStatus_IsOk(status))
        {
            return status;
        }
                                          ...
                                          uint32_t size)
{
    ...
    if ((data == NULL) || (size == 0U) || (size > STM32_ROM_BOOT_MAX_MEMORY_TRANSFER))
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
    }
    if (!FirmwareStatus_IsOk(status))
    ...
    if (!Stm32RomBoot_IsCommandSupported(device, STM32_ROM_BOOT_COMMAND_READ_MEMORY))
    {
        return FIRMWARE_STATUS_NOT_SUPPORTED;
    }
    ...
    status =
        Transmit(device, length_frame, sizeof(length_frame), device->config.command_timeout_ms);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
    ...
    return Receive(device, data, size, device->config.command_timeout_ms);
}
                                           ...
                                           const uint8_t *data, uint32_t size)
{
    ...
    if (!Stm32RomBoot_IsCommandSupported(device, STM32_ROM_BOOT_COMMAND_WRITE_MEMORY))
    {
        return FIRMWARE_STATUS_NOT_SUPPORTED;
    }
    ...
    status           = Transmit(device, frame, size + 2U, device->config.command_timeout_ms);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
                                          ...
                                          uint16_t page_count)
{
    ...
    if ((page_count == 0U) || (page_count > STM32_ROM_BOOT_MAX_ERASE_PAGE_COUNT))
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
    }
    if (!FirmwareStatus_IsOk(status))
    ...
    if (device->config.erase_mode == STM32_ROM_BOOT_ERASE_STANDARD)
    {
        ...
        if ((page_count > STM32_ROM_BOOT_STANDARD_MAX_PAGE_COUNT) ||
            ...
            !Stm32RomBoot_IsCommandSupported(device, STM32_ROM_BOOT_COMMAND_ERASE))
        {
            return FIRMWARE_STATUS_NOT_SUPPORTED;
        }
    ...
    {
        if (!Stm32RomBoot_IsCommandSupported(device, STM32_ROM_BOOT_COMMAND_EXTENDED_ERASE))
        {
            return FIRMWARE_STATUS_NOT_SUPPORTED;
        }
        status = SendCommand(device, STM32_ROM_BOOT_COMMAND_EXTENDED_ERASE);
        if (!FirmwareStatus_IsOk(status))
        {
            return status;
        }
        ...
        frame[2U + (page_count * 2U)] = XorBytes(frame, 2U + (page_count * 2U));
        frame_size                    = 3U + (uint32_t) page_count * 2U;
    }

    status = Transmit(device, frame, frame_size, device->config.command_timeout_ms);
    if (!FirmwareStatus_IsOk(status))
    {
        return status;
    }
