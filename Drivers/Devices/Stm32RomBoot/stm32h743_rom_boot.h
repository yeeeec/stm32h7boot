...
#define STM32H743_ROM_BOOT_MAX_COMMAND_COUNT  32U
    ...
    typedef stm32h743_rom_boot_status_t (*stm32h743_rom_boot_transmit_fn)(void *context,
                                                                          const uint8_t *data,
                                                                          uint32_t size,
                                                                          uint32_t timeout_ms);
    typedef stm32h743_rom_boot_status_t (*stm32h743_rom_boot_receive_fn)(void *context,
                                                                         uint8_t *data,
                                                                         uint32_t size,
                                                                         uint32_t timeout_ms);
    ...
    stm32h743_rom_boot_status_t Stm32H743RomBoot_GetInfo(stm32h743_rom_boot_t *device,
                                                         stm32h743_rom_boot_info_t *info);
    stm32h743_rom_boot_status_t Stm32H743RomBoot_Read(stm32h743_rom_boot_t *device,
                                                      uint32_t address, void *data, uint32_t size);
    /** Erases every 128-KiB sector intersecting [address, address + size). */
    stm32h743_rom_boot_status_t Stm32H743RomBoot_Erase(stm32h743_rom_boot_t *device,
                                                       uint32_t address, uint32_t size);
    /** Writes arbitrary data; address must be 4-byte aligned, final data is padded with 0xFF. */
    stm32h743_rom_boot_status_t Stm32H743RomBoot_Write(stm32h743_rom_boot_t *device,
                                                       uint32_t address, const void *data,
                                                       uint32_t size);
    stm32h743_rom_boot_status_t Stm32H743RomBoot_Verify(stm32h743_rom_boot_t *device,
                                                        uint32_t address, const void *data,
                                                        uint32_t size);
