...
int32_t GT911_GetState(GT911_Object_t *pObj, GT911_State_t *pState)
{
  ...
  if ((pObj == NULL) || (pState == NULL))
  {
    return GT911_ERROR;
  }

  pState->TouchReady = 0U;
  pState->TouchDetected = 0U;
  pState->TouchX = 0U;
  pState->TouchY = 0U;

  if (gt911_read_reg(&pObj->Ctx, GT911_TD_STAT_REG, &status, 1U) != GT911_OK)
  {
    return GT911_ERROR;
  }

  if ((status & GT911_TD_STATUS_BIT_BUFFER_STAT) == 0U)
  {
    return GT911_OK;
  }

  pState->TouchReady = 1U;
  pState->TouchDetected = (uint32_t)(status & GT911_TD_STATUS_BITS_NBTOUCHPTS);

  if (pState->TouchDetected > GT911_MAX_NB_TOUCH)
  {
    (void)gt911_write_reg(&pObj->Ctx, GT911_TD_STAT_REG, &value, 1U);
    return GT911_ERROR;
  }

  if (pState->TouchDetected > 0U)
  {
    if (gt911_read_reg(&pObj->Ctx, GT911_P1_XL_REG, data, (uint16_t)sizeof(data)) != GT911_OK)
    {
      return GT911_ERROR;
    }

    /* Send back first ready X position to caller */
    pState->TouchX = (((uint32_t)data[GT911_P1_XH_TP_BIT_POSITION] &
                       GT911_P1_XH_TP_BIT_MASK) << 8U) |
                     ((uint32_t)data[GT911_P1_XL_TP_BIT_POSITION] &
                      GT911_P1_XL_TP_BIT_MASK);
    /* Send back first ready Y position to caller */
    pState->TouchY = (((uint32_t)data[GT911_P1_YH_TP_BIT_POSITION] &
                       GT911_P1_YH_TP_BIT_MASK) << 8U) |
                     ((uint32_t)data[GT911_P1_YL_TP_BIT_POSITION] &
                      GT911_P1_YL_TP_BIT_MASK);
  }

  /* Acknowledge both touch and release frames. */
  return gt911_write_reg(&pObj->Ctx, GT911_TD_STAT_REG, &value, 1U);
}
...
int32_t GT911_GetMultiTouchState(GT911_Object_t *pObj, GT911_MultiTouch_State_t *pState)
{
  ...
  if (ret >= 0L)
  {
    pState->TouchDetected = (uint32_t)ret;
  }
  else
  {
    return ret;
