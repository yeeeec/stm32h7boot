...
typedef firmware_status_t (*image_authenticator_hash_update_fn)(void *context, const void *data,
                                                                size_t size);
/** 将 Digest 完成并写入调用者提供的 32 字节 Buffer。 */
typedef firmware_status_t (*image_authenticator_hash_finish_fn)(
    void *context, uint8_t digest[IMAGE_AUTHENTICATOR_SHA256_SIZE]);
/** 校验 Digest 签名；未知 Key 或编码必须 Fail-closed。 */
typedef firmware_status_t (*image_authenticator_verify_signature_fn)(
    void *context, const char *key_id, const uint8_t digest[IMAGE_AUTHENTICATOR_SHA256_SIZE],
