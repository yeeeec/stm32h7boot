...
typedef firmware_status_t (*package_source_is_media_present_fn)(void *context, int *present);
...
typedef firmware_status_t (*package_source_open_fn)(void *context, package_file_id_t file);
/** 关闭当前文件；只有 close 成功后 Ownership 才结束。 */
typedef firmware_status_t (*package_source_close_fn)(void *context);
/** 返回当前已打开固定文件的大小。 */
typedef firmware_status_t (*package_source_get_size_fn)(void *context, uint32_t *size);
/** 从当前已打开文件读取有界范围。 */
typedef firmware_status_t (*package_source_read_at_fn)(void *context, uint32_t offset,
