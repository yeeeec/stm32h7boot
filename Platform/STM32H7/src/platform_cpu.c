...
{
    const bool in_dtcm = (stack_pointer > PLATFORM_DTCM_BASE) &&
                         (stack_pointer <= PLATFORM_DTCM_END);
    const bool in_axi = (stack_pointer > PLATFORM_AXI_SRAM_BASE) &&
                        (stack_pointer <= PLATFORM_AXI_SRAM_END);
    const bool in_d2 = (stack_pointer > PLATFORM_D2_SRAM_BASE) &&
                       (stack_pointer <= PLATFORM_D2_SRAM_END);
    const bool in_sram4 = (stack_pointer > PLATFORM_SRAM4_BASE) &&
