...
update_result_t UpdateService_Process(void)
{
    ...
    {
#if (BOOTLOADER_UPDATE_DEBUG_MODE == 1U)
        /* JUMPING means the install and CURRENT commit completed before a
         * reset.  Debug has no APP confirmation boundary, so normalize the
         * journal and continue to the normal runtime launch path. */
        status = RuntimeVerifier_Validate();
        if (FirmwareStatus_IsError(status))
            return make_result(UPDATE_OUTCOME_RUNTIME_UNSAFE, UPDATE_FAILURE_RUNTIME_VECTOR,
                               status);
        status = UpdateJournal_WriteState(UPDATE_STATE_IDLE, UPDATE_TARGET_NONE);
        if (FirmwareStatus_IsError(status))
            return journal_error(status);
        LOG_INFO("update", "debug JUMPING confirmed: target=%u journal=IDLE/NONE",
                 (unsigned) journal.target);
