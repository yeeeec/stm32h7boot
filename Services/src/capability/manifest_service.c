...
static firmware_status_t RequireConstantBoolean(const json_document_t *document, uint32_t object,
...
static int IsValidUtcTimestamp(const json_document_t *document, uint32_t token_index)
{
    static const uint8_t separators[6]          = {'-', '-', 'T', ':', ':', 'Z'};
    static const uint8_t separator_positions[6] = {4U, 7U, 10U, 13U, 16U, 19U};
    static const uint8_t days_per_month[12]     = {31U, 28U, 31U, 30U, 31U, 30U,
    ...
    if ((token->type != JSON_TOKEN_STRING) || ((token->end - token->start) != 20U))
    {
        return 0;
    }
    ...
    return (day >= 1U) && (day <= maximum_day) && (ParseTwoDigits(&data[11]) <= 23U) &&
           (ParseTwoDigits(&data[14]) <= 59U) && (ParseTwoDigits(&data[17]) <= 59U);
}
...
static firmware_status_t ParseHex(const json_document_t *document, uint32_t token_index,
                                  uint8_t *output, uint32_t output_size, int lowercase_only)
{
    const json_token_t *token = &document->tokens[token_index];
    uint32_t index;

    if ((token->type != JSON_TOKEN_STRING) || ((token->end - token->start) != output_size * 2U))
    {
        return FIRMWARE_STATUS_INVALID_STATE;
    }
...
static firmware_status_t ParseCrc32(const json_document_t *document, uint32_t object,
                                    const char *key, uint32_t *value)
{
    uint32_t token;
    uint8_t bytes[4];
                                     ...
                                     const char *key, uint8_t output[MANIFEST_SHA256_SIZE])
{
    ...
    return FirmwareStatus_IsOk(status) ? ParseHex(document, token, output, MANIFEST_SHA256_SIZE, 1)
                                       : status;
}

static firmware_status_t ValidateReleaseGroup(const json_document_t *document, uint32_t root,
...
static firmware_status_t ValidateTransaction(const json_document_t *document, uint32_t root)
{
    static const char *const members[] = {"release_group_id", "strategy", "commit_store",
                                          "commit_condition"};
    uint32_t object;
    ...
    return (FirmwareStatus_IsOk(FindMember(document, root, "transaction", &object)) &&
            ...
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "release_group_id", "app-gui")) &&
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "strategy", "inactive-pair")) &&
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "commit_store", "eeprom")) &&
            FirmwareStatus_IsOk(RequireConstantString(document, object, "commit_condition",
                                                      "all-components-crc-valid")))
               ? FIRMWARE_STATUS_OK
               : FIRMWARE_STATUS_INVALID_STATE;
}

static firmware_status_t ValidateCrcParameters(const json_document_t *document, uint32_t root)
{
    static const char *const members[] = {"name",          "polynomial",     "initial_value",
    ...
    return (FirmwareStatus_IsOk(FindMember(document, root, "crc32_parameters", &object)) &&
            ...
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "name", "CRC-32/ISO-HDLC")) &&
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "polynomial", "0x04C11DB7")) &&
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "initial_value", "0xFFFFFFFF")) &&
            ...
            FirmwareStatus_IsOk(
                RequireConstantString(document, object, "xor_output", "0xFFFFFFFF")))
               ? FIRMWARE_STATUS_OK
               : FIRMWARE_STATUS_INVALID_STATE;
}

static firmware_status_t ParseAppComponent(const json_document_t *document, uint32_t object,
                                           manifest_app_component_t *app)
{
    static const char *const members[]            = {"id",
                                                     "file",
                                                     "format",
                                                     "target",
    ...
    if (!FirmwareStatus_IsOk(ValidateObjectMembers(document, object, members, 14U)) ||
        ...
        !FirmwareStatus_IsOk(
            RequireConstantString(document, object, "format", "raw-xip-reloc-v2")) ||
        !FirmwareStatus_IsOk(
            RequireConstantString(document, object, "target", "inactive-app-slot")) ||
        !FirmwareStatus_IsOk(RequireConstantU32(document, object, "maximum_image_size_bytes",
                                                APP_MAXIMUM_IMAGE_SIZE)) ||
        !FirmwareStatus_IsOk(
            RequireU32(document, object, "file_size_bytes", &app->file_size_bytes)) ||
        (app->file_size_bytes == 0U) ||
        !FirmwareStatus_IsOk(
            RequireU32(document, object, "image_size_bytes", &app->image_size_bytes)) ||
        ...
        !FirmwareStatus_IsOk(
            RequireConstantString(document, relocation, "file", "hmi.app.reloc.bin")) ||
        ...
        (app->relocation_count == 0U) || (app->relocation_count > APP_MAXIMUM_RELOCATIONS) ||
...
static firmware_status_t ParseGuiComponent(const json_document_t *document, uint32_t object,
                                           manifest_gui_component_t *gui)
{
    static const char *const members[] = {
        "id",    "file",  "format", "target", "maximum_image_size_bytes", "file_size_bytes",
        "crc32", "sha256"};
    ...
    if (!FirmwareStatus_IsOk(ValidateObjectMembers(document, object, members, 8U)) ||
        ...
        !FirmwareStatus_IsOk(
            RequireConstantString(document, object, "target", "inactive-gui-slot")) ||
        !FirmwareStatus_IsOk(RequireConstantU32(document, object, "maximum_image_size_bytes",
                                                GUI_MAXIMUM_IMAGE_SIZE)) ||
        !FirmwareStatus_IsOk(
            RequireU32(document, object, "file_size_bytes", &gui->file_size_bytes)) ||
...
static firmware_status_t ValidateRoot(const json_document_t *document,
                                      validated_manifest_t *manifest)
{
    static const char *const members[] = {"format_version",
                                          "package_id",
                                          "product_id",
                                          "hardware_id",
                                          "build_number",
    ...
    if (!FirmwareStatus_IsOk(ValidateObjectMembers(document, 0U, members, 12U)) ||
        ...
        !FirmwareStatus_IsOk(
            RequireConstantString(document, 0U, "hardware_id", "STM32H743-W25Q256")) ||
