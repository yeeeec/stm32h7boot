    ...
    uint32_t entry_count)
{
    ...
    if ((service->initialized == 0) ||
        (block_offset != service->next_block_offset))
    {
    ...
    for (index = 0U; index < entry_count; ++index)
    {
        ...
        if ((entry->type != APPX_RELOCATION_ABS32_ADD_XIP_BASE) ||
            ...
            ((validation_has_relocation != 0) &&
             (entry->target_offset <= validation_last_offset)))
        {
            return FIRMWARE_STATUS_INVALID_STATE;
        }
        if (!CheckedArithmetic_AddU32(
                entry->target_offset, sizeof(uint32_t), &entry_end) ||
    ...
    if (entry_count != 0U)
    {
        service->last_relocation_offset =
            entries[entry_count - 1U].target_offset;
    ...
    uint32_t expected_relocations)
{
    ...
    if ((service->initialized == 0) ||
        (service->next_block_offset != service->image_size) ||
