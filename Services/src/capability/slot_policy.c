...
firmware_status_t SlotPolicy_GetPairLayout(boot_pair_t pair, boot_pair_layout_t *layout)
{
    if (layout == NULL)
    {
...
firmware_status_t SlotPolicy_SelectInactivePair(boot_pair_t active_pair, boot_pair_t *inactive_pair)
{
...
firmware_status_t SlotPolicy_ValidateImageSize(const boot_region_t *region, uint32_t image_size)
{
    if (region == NULL)
    {
        return FIRMWARE_STATUS_INVALID_ARGUMENT;
...
int SlotPolicy_ContainsRange(const boot_region_t *region, uint32_t flash_offset, uint32_t size)
{
    uint32_t region_end;
    uint32_t range_end;

    if ((region == NULL) ||
        !CheckedArithmetic_RangeEndU32(region->flash_offset, region->capacity_bytes, &region_end) ||
...
firmware_status_t SlotPolicy_ValidateStorageGeometry(uint32_t capacity_bytes, uint32_t erase_size)
{
