...
static firmware_status_t Fail(
    launch_service_t *service,
    firmware_status_t status,
    boot_error_t error,
    launch_stage_t stage)
{
    ...
    const launch_service_dependencies_t *dependencies)
{
    if ((service == NULL) || (dependencies == NULL) ||
        ...
        (dependencies->xip_controller == NULL) ||
        ...
        (dependencies->application_jump->execute == NULL) ||
    ...
    const boot_active_record_t *active_record)
{
    ...
    if (!FirmwareStatus_IsOk(status) ||
        !FirmwareStatus_IsOk(
            SlotPolicy_ValidateImageSize(&layout.app, active_record->app_size)))
    {
        return Fail(
            service,
    ...
    status = service->storage->read(
        ...
        layout.app.flash_offset,
        vector_bytes,
        sizeof(vector_bytes));
    if (!FirmwareStatus_IsOk(status))
    {
        return Fail(
            service, status, BOOT_ERROR_MEDIA_UNAVAILABLE, LAUNCH_STAGE_READ_VECTOR);
    }
    ...
    status = VectorValidation_Validate(
        &vectors,
        &layout.app,
        active_record->app_size,
        service->sram_regions,
        service->sram_region_count);
    if (!FirmwareStatus_IsOk(status))
    {
        return Fail(
            service, status, BOOT_ERROR_VECTOR_TABLE, LAUNCH_STAGE_VALIDATE_VECTOR);
    }

    status = service->xip_controller->enter_memory_mapped_read(
    ...
    {
        return Fail(
            service, status, BOOT_ERROR_XIP_SETUP, LAUNCH_STAGE_INVALIDATE_CACHE);
    }

    status = service->application_jump->execute(
        service->application_jump->context, layout.app.mapped_address);
    return Fail(
        service,
...
const service_result_t *LaunchService_GetResult(
    const launch_service_t *service)
{
