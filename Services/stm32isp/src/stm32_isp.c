...
static stm32_isp_status_t uart_write(stm32_isp_t *isp,
                                     const uint8_t *data,
                                     size_t length,
                                     uint32_t timeout_ms)
{
    return port_result(isp->port.uart_write(data,
                                            length,
                                            timeout_ms,
                                            isp->port.user_context));
}

static stm32_isp_status_t uart_read(stm32_isp_t *isp,
                                    uint8_t *data,
                                    size_t length,
                                    uint32_t timeout_ms)
{
    return port_result(isp->port.uart_read(data,
                                           length,
                                           timeout_ms,
                                           isp->port.user_context));
}
...
static bool command_present(const uint8_t *commands,
                            size_t count,
                            uint8_t command)
{
    size_t i;
...
{
    if ((isp == NULL) || (port == NULL) ||
        (port->uart_write == NULL) || (port->uart_read == NULL) ||
        (port->boot0_write == NULL) || (port->nrst_write == NULL) ||
        (port->delay_ms == NULL)) {
                                          ...
                                          size_t *command_count)
{
    ...
    if ((isp == NULL) || (bootloader_version == NULL) ||
        (command_count == NULL) ||
    ...
    status = uart_read(isp, response, response_length,
                       isp->command_timeout_ms);
    if (status != STM32_ISP_OK) {
        return status;
    }
    status = wait_ack(isp, isp->command_timeout_ms);
    ...
    *command_count = available_commands;
    copy_count = (available_commands < command_capacity) ?
                 available_commands : command_capacity;
    if ((commands != NULL) && (copy_count != 0u)) {
        memcpy(commands, &response[1], copy_count);
    }

    isp->supports_read_memory = command_present(&response[1],
                                                available_commands,
                                                STM32_ISP_CMD_READ_MEMORY);
    isp->supports_erase = command_present(&response[1],
                                         available_commands,
                                         STM32_ISP_CMD_ERASE);
    isp->supports_extended_erase = command_present(&response[1],
                                                  available_commands,
                                                  STM32_ISP_CMD_EXTENDED_ERASE);
...
stm32_isp_status_t stm32_isp_get_id(stm32_isp_t *isp, uint16_t *product_id)
{
    ...
    *product_id = (uint16_t)(((uint16_t)id[id_length - 2u] << 8) |
                             id[id_length - 1u]);
    return STM32_ISP_OK;
}
                                         ...
                                         size_t length)
{
    ...
    if ((isp == NULL) || (data == NULL) ||
        (length == 0u) || (length > STM32_ISP_MAX_WRITE_SIZE)) {
        return STM32_ISP_ERROR_INVALID_ARGUMENT;
    }
    ...
    status = uart_write(isp, length_packet, sizeof(length_packet),
                        isp->command_timeout_ms);
    if (status != STM32_ISP_OK) {
        return status;
    }
                                          ...
                                          size_t length)
{
    ...
    status = uart_write(isp, packet, length + 2u,
                        isp->command_timeout_ms);
    if (status != STM32_ISP_OK) {
        return status;
    }
...
static stm32_isp_status_t extended_mass_erase(stm32_isp_t *isp)
{
    ...
    status = uart_write(isp, packet, sizeof(packet),
                        isp->command_timeout_ms);
    if (status != STM32_ISP_OK) {
        return status;
    }
                                           ...
                                           bool verify_after_write)
{
    ...
    if ((isp == NULL) || (image == NULL) || (image_size == 0u) ||
        ((start_address & 3u) != 0u) ||
    ...
    while (offset < image_size) {
        ...
        status = stm32_isp_write_memory(isp, address,
        ...
        if (verify_after_write) {
            status = stm32_isp_read_memory(isp, address,
                                           verify_buffer, write_length);
            if (status != STM32_ISP_OK) {
                return status;
            }
