/**
 * @file logging.h
 * @brief Formatted logging over caller-provided time and byte-output ports.
 *
 * Output is synchronous and may block in the configured writer. No log task
 * or caller-owned buffer is involved.
 */
...
/**
 * Format one log line and synchronously send it through the configured writer.
 * Messages written before initialization are discarded.
 */
...
#define LOG_ERROR(tag, format, ...) Logging_Write(LOGGING_LEVEL_ERROR, tag, format, ##__VA_ARGS__)
#else
#define LOG_ERROR(...) ((void) 0)
#endif

#if FIRMWARE_LOG_LEVEL >= FIRMWARE_LOG_LEVEL_WARN
#define LOG_WARN(tag, format, ...) Logging_Write(LOGGING_LEVEL_WARN, tag, format, ##__VA_ARGS__)
#else
#define LOG_WARN(...) ((void) 0)
#endif

#if FIRMWARE_LOG_LEVEL >= FIRMWARE_LOG_LEVEL_INFO
#define LOG_INFO(tag, format, ...) Logging_Write(LOGGING_LEVEL_INFO, tag, format, ##__VA_ARGS__)
#else
#define LOG_INFO(...) ((void) 0)
#endif

#if FIRMWARE_LOG_LEVEL >= FIRMWARE_LOG_LEVEL_DEBUG
#define LOG_DEBUG(tag, format, ...) Logging_Write(LOGGING_LEVEL_DEBUG, tag, format, ##__VA_ARGS__)
#else
#define LOG_DEBUG(...) ((void) 0)
#endif

/* Panic messages bypass the selected level but still require logging enabled. */
#define LOG_PANIC(tag, format, ...) Logging_Write(LOGGING_LEVEL_ERROR, tag, format, ##__VA_ARGS__)
...
#define LOG_ERROR(tag, format, ...) LOG_DISABLED(LOGGING_LEVEL_ERROR, tag, format, ##__VA_ARGS__)
#define LOG_WARN(tag, format, ...)  LOG_DISABLED(LOGGING_LEVEL_WARN, tag, format, ##__VA_ARGS__)
#define LOG_INFO(tag, format, ...)  LOG_DISABLED(LOGGING_LEVEL_INFO, tag, format, ##__VA_ARGS__)
#define LOG_DEBUG(tag, format, ...) LOG_DISABLED(LOGGING_LEVEL_DEBUG, tag, format, ##__VA_ARGS__)
#define LOG_PANIC(tag, format, ...) LOG_DISABLED(LOGGING_LEVEL_ERROR, tag, format, ##__VA_ARGS__)
